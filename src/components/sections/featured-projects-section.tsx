...
export function FeaturedProjectsSection() {
  ...
  return (
    <section id="featured-projects" className="relative border-b border-white/10 py-24 md:py-32">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_80%_20%,rgba(6,182,212,0.12),transparent_55%)]" />
      <div className="relative mx-auto max-w-6xl px-6">
        <motion.div
          ...
          className="max-w-2xl"
        >
          <p className="text-xs font-semibold tracking-[0.35em] text-purple-300/80">FEATURED WORK</p>
          ...
          <p className="mt-3 text-sm leading-relaxed text-zinc-300 md:text-base">
            Expand each project to see the architectural decisions, technical challenges, and full stack implementation.
          </p>
        </motion.div>

        <div className="relative mt-14 md:pl-8">
                      ...
                      >
                        <div className="mt-3 rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.06] to-transparent p-5 text-sm leading-relaxed text-zinc-300 backdrop-blur-md">
                          <ul className="ml-4 list-disc space-y-2 text-zinc-300">
                            {Array.isArray(proj.summary) ? proj.summary.map((point, i) => (
                              <li key={i}>{point}</li>
                            )) : <li>{proj.summary}</li>}
                          </ul>
